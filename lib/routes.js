...
FlowRouter.route('/admin/fields', {
    action: function (params) {
        BlazeLayout.render('AdminLayout', {main: 'Fields'});
    }
});

//demo version
if(Meteor.settings.public.demo) {
    FlowRouter.route('/demo/:userId', {
        action: (params, queryParams) => {
            Meteor.call('users.createToken', params.userId, (error, stampedLoginToken)=>{
                Accounts.loginWithToken(stampedLoginToken);
            });
            FlowRouter.go('/');
        }
    });
